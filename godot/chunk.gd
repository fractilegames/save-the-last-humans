...
func set_size(p_chunk_size: int, p_square_size: int) -> void:
	...
	for i in range(chunk_size):
		for j in range(chunk_size):
			...
			for midpoint in TRIANGLE_LOOKUP[case]:
				...
				mesh_colors.append(Color(0.0, 0.4, 0.0, 1.0))
...
#					new_occluder.occluder = OccluderPolygon2D.new()
#					new_occluder.occluder.polygon = PoolVector2Array(masses[i])
#					$StaticBody2D.add_child(new_occluder)
